...
local function isContainerOrDoor(reference)
    return (reference.object.objectType == tes3.objectType.container or reference.object.objectType == tes3.objectType.door)
end
...
local function objectTooltip(e)
        ...
        for i = 1, #trap.effects do
            ...
            if effect.id >= 0 then
                local attributeOrSkillName =
                    tes3.attributeName[effect.attribute] or tes3.skillName[effect.skill]
    ...
    if config.showTrapEnchantmentEffect == "Target" and getTrap(e.reference) then
